    ...
    chat_template = ChatPromptTemplate.from_messages(
        [
            (
                "user",
                """
                Você é um assistente que responde perguntas de videos do youtube baseados na transcrição do video.

                Responda a seguinte pergunta: {pergunta}
                Procurando nas seguintes transcrições: {docs}

                Use somente informação da transcrição para responder a perunta. Se vocẽ não sabe, responda com "Eu não sei"
...
if __name__ == "__main__":
    db = create_vector_from_yt_url("https://www.youtube.com/watch?v=UWT2RHA0M-c")
    response, docs = get_response_from_query(
        db, "qual assunto do video?"
    )
